...
import           TokenSale (tokenSaleScript, tokenSaleScriptShortBs)
...
main = do
  ...
  let scriptname = if nargs > 1 then args!!1 else  "token_sale.plutus"
  putStrLn $ "OUTPUT: " ++ scriptname
  writePlutusScript scriptnum scriptname tokenSaleScript tokenSaleScriptShortBs
